...
public class AP_Home extends javax.swing.JPanel {
   ...
   public void loadTableData() {
    String sql = """
        SELECT p.nama AS pasien, jp.tanggal, jp.status 
        FROM jadwal_perawatan jp
        JOIN pasien p ON jp.id_pasien = p.id_pasien
        ORDER BY 
            CASE 
                WHEN jp.tanggal = CURDATE() THEN 1
                ELSE 2
            END, 
            jp.tanggal DESC
    """;

    try (Connection conn = DriverManager.getConnection("jdbc:mysql://localhost:3306/medica", "root", "");
         Statement stmt = conn.createStatement();
         ResultSet rs = stmt.executeQuery(sql)) {

        // DefaultTableModel untuk menampilkan data di JTable
        DefaultTableModel model = new DefaultTableModel(new String[]{"Nama", "Tanggal", "Status"}, 0);

        // Memuat data dari ResultSet ke DefaultTableModel
        while (rs.next()) {
            String nama = rs.getString("pasien");
            String tanggal = rs.getString("tanggal");
            String status = rs.getString("status");
            model.addRow(new Object[]{nama, tanggal, status});
        }

        // Menampilkan data ke JTable
        Jadwal_Perawatan.setModel(model);

    } catch (SQLException ex) {
        ex.printStackTrace();
        JOptionPane.showMessageDialog(this, "Gagal memuat data pasien: " + ex.getMessage());
    }
}
